...
export default class OnlineStatus extends Component {
    render() {
        let onlinePeople = Object.keys(this.props.currentRoom.members).map((member, index) => {
            ...
            if (member.account !== this.props.account) {
                ...
                return (
                    <li key={index} className="list-group-item border-0 bg-dark text-light">
                        <i className={"fa fa-circle " + cls} /> {member.account}
                    </li>
                );
            }
        });
        return (
            <>
                ...
                <div className="list-group">{onlinePeople}</div>
            </>
        );
    }
}
